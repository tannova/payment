...
func (tx *Tx) init() {
	tx.CryptoWallet = NewCryptoWalletClient(tx.config)
	tx.MerchantUserBankAccount = NewMerchantUserBankAccountClient(tx.config)
	tx.Payment = NewPaymentClient(tx.config)
	tx.PaymentBankingDetail = NewPaymentBankingDetailClient(tx.config)
	tx.PaymentCryptoDetail = NewPaymentCryptoDetailClient(tx.config)
	tx.PaymentEWalletDetail = NewPaymentEWalletDetailClient(tx.config)
	tx.PaymentTelcoDetail = NewPaymentTelcoDetailClient(tx.config)
	tx.Revision = NewRevisionClient(tx.config)
	tx.Setting = NewSettingClient(tx.config)
	tx.SystemBankAccount = NewSystemBankAccountClient(tx.config)
	tx.SystemCryptoHotWallet = NewSystemCryptoHotWalletClient(tx.config)
	tx.SystemEWallet = NewSystemEWalletClient(tx.config)
}
...
// applies a query, for example: CryptoWallet.QueryXXX(), the query will be executed
