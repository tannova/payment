...
func (rq *RevisionQuery) QueryPayment() *PaymentQuery {
...
func (rq *RevisionQuery) FirstID(ctx context.Context) (id int64, err error) {
	var ids []int64
...
func (rq *RevisionQuery) FirstIDX(ctx context.Context) int64 {
...
func (rq *RevisionQuery) OnlyID(ctx context.Context) (id int64, err error) {
	var ids []int64
...
func (rq *RevisionQuery) OnlyIDX(ctx context.Context) int64 {
...
func (rq *RevisionQuery) IDs(ctx context.Context) ([]int64, error) {
	var ids []int64
...
func (rq *RevisionQuery) IDsX(ctx context.Context) []int64 {
	ids, err := rq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (rq *RevisionQuery) WithPayment(opts ...func(*PaymentQuery)) *RevisionQuery {
	query := &PaymentQuery{config: rq.config}
...
func (rq *RevisionQuery) sqlAll(ctx context.Context) ([]*Revision, error) {
	...
	if query := rq.withPayment; query != nil {
		...
		nodeids := make(map[int64][]*Revision)
		for i := range nodes {
			if nodes[i].payment_revisions == nil {
				continue
			}
			fk := *nodes[i].payment_revisions
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "payment_revisions" returned %v`, n.ID)
			}
			for i := range nodes {
