...
func (s *morganServer) RejectTelcoWithdraw(ctx context.Context, request *stark.RejectTelcoWithdrawRequest) (*stark.RejectTelcoWithdrawReply, error) {
	...
	err = tx.WithTransaction(ctx, logger, s.entClient, func(ctx context.Context, tx tx.Tx) error {
		payment, err := tx.Client().Payment.
			...
			First(ctx)
		if err != nil {
			logger.Error("failed to get telco withdraw payment by id", zap.Error(err))
			return err
		}
		...
		payment, err = tx.Client().Payment.UpdateOne(payment).
			...
			Save(ctx)
		if err != nil {
			logger.Error("failed to update telco withdraw payment status", zap.Error(err))
			return err
		}
	...
	return &stark.RejectTelcoWithdrawReply{}, nil
}
