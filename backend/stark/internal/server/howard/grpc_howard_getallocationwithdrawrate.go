...
func (s *howardServer) GetAllocationWithdrawRate(ctx context.Context, request *stark.GetReportRequest) (*stark.GetAllocationWithdrawRateReply, error) {
	...
	logger.Info("GetAllocationWithdrawRate", zap.Any("request", request))

	stats, err := s.getMethodAmountStats(ctx, from, to, request.GetMerchantId(), stark.PaymentType_WITHDRAW)
	if err != nil {
	...
	return &stark.GetAllocationWithdrawRateReply{
		TotalWithdraw: total,
		WithdrawAllocationRate: []*stark.AllocationDetail{
