...
func (s *howardServer) GetTopTeller(ctx context.Context, request *stark.GetReportRequest) (*stark.GetTopTellerReply, error) {
	...
	logger.Info("GetTopTeller", zap.Any("request", request))
	...
	err := s.entClient.Debug().Payment.Query().
		...
		Modify(func(s *sql.Selector) {
			...
			s.
				Select(
					sql.As(entpm.FieldApprovedBy, "user_id"),
				...
				GroupBy(s.C(entpm.FieldApprovedBy)).
	...
	return &stark.GetTopTellerReply{TopTellerRevenue: data}, nil
}
