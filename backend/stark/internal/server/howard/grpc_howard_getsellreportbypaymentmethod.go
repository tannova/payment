...
func (s *howardServer) GetSellReportByPaymentMethod(ctx context.Context, request *stark.GetSellReportByPaymentMethodRequest) (*stark.GetSellReportByPaymentMethodReply, error) {
	...
	var (
		...
		data      []struct {
			PaymentMethod int32   `json:"method"`
	...
	logger.Info("GetSellReportByPaymentMethod", zap.Any("request", request))
	...
	err := s.entClient.Debug().Payment.Query().
		...
		Modify(func(s *sql.Selector) {
			...
			s.
				...
				GroupBy(s.C(entpm.FieldMethod), s.C(entpm.FieldType))
		}).
		Scan(ctx, &data)
	if err != nil {
	...
	for _, d := range data {
		item := &stark.SaleReportItem{
			Key: &stark.SaleReportItem_PaymentMethod{
				PaymentMethod: stark.MethodType(d.PaymentMethod),
			},
			Quantity: d.Quantity,
			Amount:   d.Amount,
			Discount: d.Discount,
	...
	return &stark.GetSellReportByPaymentMethodReply{
		TopUps:    topUps,
		Withdraws: withdraws,
	}, nil
}
