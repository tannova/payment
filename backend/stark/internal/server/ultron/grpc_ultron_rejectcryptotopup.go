...
func (s *ultronServer) RejectCryptoTopUp(ctx context.Context, request *stark.RejectCryptoTopUpRequest) (*stark.RejectCryptoTopUpReply, error) {
	...
	err = tx.WithTransaction(ctx, logger, s.entClient, func(ctx context.Context, tx tx.Tx) error {
		...
		err = s.jarvisClient.CompletePayment(
			...
			completeRequest,
			s.completeRejectCryptoTopupCallback)
	...
	return &stark.RejectCryptoTopUpReply{}, nil
}

func (s *ultronServer) completeRejectCryptoTopupCallback(
