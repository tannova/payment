...
PUBLISHED=`readlink -e ${PWD}/Makefile`
echo "${PUBLISHED}" > publishedlist
...
publish-proto () {
  ...
  if [[ $PUBLISHED != *"${MKFILE_FULL}"* ]]; then
    echo "Build ${DIRNAME} (${MKFILE_FULL})"

    # Main build command.
    INCLUDE_MAKEFILE=$MKFILE make publish-proto
    ...
    PUBLISHED=`echo "${PUBLISHED};${MKFILE_FULL}"`
    echo "${PUBLISHED}" > publishedlist
  else
